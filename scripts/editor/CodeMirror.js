...
CodeMirror.defineMode(
  ...
  function (cmCfg, modeCfg) {
    const htmlFound = CodeMirror.modes.hasOwnProperty('xml')
    const htmlMode = CodeMirror.getMode(
      cmCfg,
    ...
    const quote1 = 'atom'
    const quote2 = 'number'
    const list1 = 'variable-2'
    const list2 = 'variable-3'
    const list3 = 'keyword'
    const hr = 'hr'
    const image = 'tag'
    const linkinline = 'link'
    const linkemail = 'link'
    const linktext = 'link'
    ...
    const ulRE = /^[*\-+]\s+/
    const olRE = /^[0-9]+\.\s+/
    const taskListRE = /^\[(x| )\](?=\s)/ // Must follow ulRE or olRE
    const headerRE = /^(?:\={1,}|-{1,})$/
    const textRE = /^[^!\[\]*_\\<>` "'(]+/
    ...
    function htmlBlock(stream, state) {
      ...
      if (state.md_inside && stream.current().includes('>')) {
        state.f = inlineNormal
        state.block = blockNormal
    ...
    function inlineNormal(stream, state) {
      ...
      if (ch === '<' && stream.match(/^\w/, false)) {
        if (stream.string.includes('>')) {
          const atts = stream.string.substring(1, stream.string.indexOf('>'))
      ...
      if (!modeCfg.underscoresBreakWords) {
        if (
          ...
          const prevPos = stream.pos - 2
          if (prevPos >= 0) {
            const prevCh = stream.string.charAt(prevPos)
